...
two <- tweets %>%
  ...
  select(user_id, date,time, tweet)

two %>%
  filter(user_id== 'KJacobs27')


multiTweeterse %>%
  left_join(tweets, by = "user_id") %>%
...
#1  
parttimers <- CPS1988[CPS1988$parttime == 'yes', c('wage', 'education', 'experience')]
...
CPS1988 %>%
  filter(wage > mean(wage)) %>%
  summarise(log = log(experience)) %>%
  mean()

newChart <- CPS1988 %>%
  mutate(educLevel = NA) %>%
  mutate(educLevel = replace(educLevel, which(education >= 0 &education<12), "HS dropout")) %>%
  mutate(educLevel = replace(educLevel, which(education>=12 & education<16), "HS Grad")) %>%
  mutate(educLevel = replace(educLevel, which(education>=16&education<=18), "College Grad")) %>%
  group_by(educLevel) %>%
